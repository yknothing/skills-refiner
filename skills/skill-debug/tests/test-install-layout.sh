...
make_no_jq_path() {
    ...
    local tool path
    for tool in dirname basename awk date mkdir head; do
...
run_tests() {
    ...
    assert_jq "scan schema is v3" "$scan_json" '.metadata.schema_version == "skill-scan.v3"'
    ...
    assert_eq "Trace block stays small" "yes" "$([ "$block_lines" -le 12 ] && echo yes || echo no)"
    ...
    local no_jq_bin no_jq_event no_jq_identity no_jq_rc no_jq_stderr bash_bin
