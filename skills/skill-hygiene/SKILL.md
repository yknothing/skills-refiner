...
For ordinary health review, continue with the scan and analysis below. Load
specialized procedures only when the task needs them:

- For a declared physical collection upgrade, repair, recovery or undo, read
  [collection operations](references/collection-operations.md). It covers the
  managed profiles and the separate ProdCraft V1 compatibility path. Use the
  existing transaction controller, exact source revision and returned plan or
  operation identity; preserve unqualified collisions and recovery evidence.
- For host exposure changes or native runtime observations, read
  [runtime operations](references/runtime-operations.md). Physical layout,
  deployment policy, native catalog, body access, routing and context are
  independent facts. Current adapters record catalog evidence only; body and
  route remain unverified. Never label that result runtime qualification.

Single-entry retirement uses the cleanup flow above. A health assessment or
collection upgrade does not implicitly authorize retirement or host changes.
Carry forward authorization for the same scope; ask only for missing authority
or an exact confirmation required by the selected mutation tool.
...
### 4. Size & Context Cost
Examine whether content is needed for the current task and available when used.
- Length alone does not establish waste or justify removal.
- Preserve useful expertise, examples, recovery steps and rare critical behavior.
- Move substantial mode-specific procedures to reachable references when they
  burden unrelated tasks; verify that the consuming workflow can still use them.
- Claim cost savings only with measured runtime evidence, not file length.
...
- Generated and hand-authored content use the same value standard: inspect its
  purpose, consumers, current correctness and replacement/recovery path.
- Authorship, age and low observed use do not establish disposability. Preserve
  useful domain knowledge and rare critical workflows regardless of origin.
