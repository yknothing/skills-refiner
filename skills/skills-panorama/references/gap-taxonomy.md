...
| 命名冲突 | `name_collision` | 同一宿主加载空间内已证实的同名歧义或覆盖 | `collision.status=conflict` |
...
- **分类优先级：** 链接损坏 > 已确认命名冲突 > 清单与现实不符 > 同名记录的加载影响未知 > 部分 Agent 已出现 > 仅在 Agent > 仅在源目录 > 暂无法判定 > 齐全。

静态发现多条同名记录时保留所有实际目标，并归入「暂无法判定」以表明宿主加载影响未验证。`collision.status=none` 只表示本次本地记录未触发同名差异检查，不证明运行时没有冲突。
